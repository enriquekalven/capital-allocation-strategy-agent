...
from capital_allocation_strategy_agent.shared_libraries.request_id_utils import (
...
async def extract_request_id_from_request(
    ...
    try:
        ...
        try:
            # Allow general questions to pass without a STRAT- ID scenario
            lowered_message = user_message.lower().strip()
            general_q = ["what can you do", "help", "who are you", "hello", "hi", "capabilities", "features"]
            if any(q in lowered_message for q in general_q):
                logger.info("General question detected, bypassing scenario ID check.")
                return None
            ...
            # Auto-generate a default scenario ID for demo purposes so user doesn't need to type it!
