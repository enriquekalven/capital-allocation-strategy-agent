...
OUTPUT_KEY_MAP = {
    "MarketIntelligenceAgent": "MarketIntelligenceAgent_output",
    "PortfolioRiskAgent": "PortfolioRiskAgent_output",
    "CapitalStructureAgent": "CapitalStructureAgent_output",
    "GovernanceAuditorAgent": "GovernanceAuditorAgent_output",
}
...
def check_process_status(tool_context: ToolContext) -> dict:
    ...
    try:
        ...
        logger.warning(f"Firestore not available, falling back to local in-memory state: {e}")
        # Return success to let the agent continue locally if Firestore isn't provisioned!
        return {
            "status": "initialized",
            "action": "proceed_to_analysis",
...
def nano_banana_image_gen(tool_context: ToolContext, prompt: str) -> dict:
    """
    Generate high-fidelity marketing concepts for the board to approve the brand direction of a new segment.
    Uses the Nano Banana Image Gen engine.

    Args:
        prompt: The visual description of the concept to generate.
    """
    try:
        logger.info(f"Generating image concept with Nano Banana: {prompt}")
        return {
            "status": "success",
            "message": f"Generated high-fidelity concept for board review: '{prompt}'",
            "concept_url": f"https://mock-image-gen.cymbal.com/concept/{prompt.replace(' ', '-').lower()}.png",
            "framing": "High-fidelity marketing concepts for visual validation by the Executive Committee.",
        }
    except Exception as e:
        logger.error(f"Error in nano_banana_image_gen: {e}")
